        ...
        public static IHostBuilder UseSqlite(this IHostBuilder hostBuilder)
        {
            return hostBuilder
                .ConfigureServices(
                    (hostingContext, services) =>
                    {
                        services.AddDbContext<TadmorDbContext>(
                            optionsBuilder =>
                            {
            ...
            return hostBuilder
                .ConfigureServices(
                    services =>
                    {
                        ...
                        services.AddTransient(p => p.GetRequiredService<INotificationPublisherFactory>().Create());
                        ...
                        services.AddTransient(
                            s => s.GetRequiredService<ICommandContextResolver>().CurrentCommandContext);
                        services
                            .AddTransient<INotificationHandler<MessageReceivedNotification>, ConsideredMessagesFilter
                            >();

                        // if the order is changed, a rule may be triggered as it's being added through a command
                        services
                            .AddTransient<INotificationHandler<MessageValidatedNotification>, TriggeredChatRulesBroker
                            >();
                        services
                            .AddTransient<INotificationHandler<MessageValidatedNotification>,
                                MessageDrivenCommandExecutor
                            >();
                        ...
                        services
                            .AddTransient<INotificationHandler<GuildMemberUpdatedNotification>,
                                CachedContextualPreferencesProviderDecorator>();
                        services
                            .AddTransient<INotificationHandler<GuildPreferencesUpdatedNotification>,
                                CachedContextualPreferencesProviderDecorator>();
                    })
                .UseSqlite();
        }
